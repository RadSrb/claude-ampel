...
const RANK = { limit: 0, attention: 1, question: 2, stalled: 3, running: 4, done: 5, unknown: 6 };
...
export class StateStore {
  ...
  #row(session, now) {
    ...
    // Das Limit schlaegt jede Hook-Meldung: der Stop-Hook meldet pflichtgemaess
    // "Turn zu Ende" und faerbt gruen -- er kann nicht wissen, dass Claude
    // gegen eine Wand gelaufen ist.
    if (derived.status === 'limit') status = 'limit';
...
function deriveFromTranscript(session) {
  ...
  // Das Nutzungslimit beendet den Zug mit stop_sequence, nicht mit end_turn --
  // ohne eigene Pruefung liefe das als "mittendrin" durch. Es ist aber weder
  // fertig noch laufend, sondern blockiert.
  if (tail.limit) {
    const bis = tail.limit.zurueckUm ? `zurueck um ${tail.limit.zurueckUm}` : null;
    return { status: 'limit', note: bis, lastActivity, source: 'derived' };
  }
